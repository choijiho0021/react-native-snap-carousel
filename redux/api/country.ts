...
const splitCountry = (prodCountry: string[]) => {
  prodCountry.forEach((line) => {
    const seg = line.split(',');
    ccode[seg[0].toLowerCase()] = {
      ko: seg[1].split(':'),
      en: seg[2].split(':'),
...
const getCode = (name: string, lang = 'ko', prodCountry: string[]) => {
  splitCountry(prodCountry);
...
const getPfx = (prodCountry: string[], code?: string) => {
  splitCountry(prodCountry);
...
const getCountryName = (code?: string, lang = 'ko') => {
  ...
  return ccode[cc] === undefined ? 'N/A' : ccode[cc][lang].join(',');
};

const getName = (code: string[] = [], lang = 'ko', prodCountry: string[]) => {
  splitCountry(prodCountry);
