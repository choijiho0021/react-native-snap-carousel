...
class ChargeSummary extends PureComponent {
  render() {
    const {totalCnt=0, totalPrice=0, balance=0, dlvCost=0, simList} = this.props,
          // 상품가격 + 배송비
    ...
    return (
      ...
      {
        !_.isEmpty(simList) &&
        <LabelText label={i18n.t('cart:dlvCostNotice')} labelStyle={{color:colors.warmGrey}} value=""/>
      }

        <LabelText label={i18n.t('cart:totalCnt')} style={[styles.summary, styles.summaryTop]} valueStyle={{...appStyles.normal14Text,color:colors.black}}
