...
class PurchaseDetailScreen extends Component {
  ...
  _paymentInfo() {
    ...
    const isUsed =
      (!_.isEmpty(usageList) &&
        usageList.find(value => value.status != 'R' && value.status != 'I')) ||
      false;
  ...
  _headerInfo() {
    const {orderNo, orderDate, orderItems, isCanceled, method, disableBtn} =
      this.state || {};
    ...
    if (orderItems.length > 1)
      label = label + i18n.t('his:etcCnt').replace('%%', orderItems.length - 1);
